...
#define GREEN_PIN 0   // Green LED pin
#define BLUE_PIN 4    // Blue LED pin

bool led_mode = true; // Default to color mode active
unsigned long button_press_start = 0; // To track button press duration
...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
    void onResult(BLEAdvertisedDevice advertisedDevice) {
        ...
        if (led_mode) {
            flash_led(BLUE_PIN); // Flash blue for Bluetooth
        }
    }
};
...
void setup() {
    ...
    display.display(); // Ensure the display is updated with the text

    Serial.println("Started");
    Serial1.println("REV3!");

    pinMode(BUTTON_PIN, INPUT_PULLUP); // Set button pin as input
    pinMode(RED_PIN, OUTPUT);           // Set LED pins as output
...
void loop() {
    ...
    for (int y = 0; y < 6; y++) {
        ...
        if (n > 0) {
            for (int i = 0; i < n; i++) {
                ...
                if (led_mode) {
                    // Flash purple for each Wi-Fi signal found (both red and blue)
                    flash_led(GREEN_PIN);   // Flash red
                }
            }
        }
    }
    ...
    if (digitalRead(BUTTON_PIN) == LOW) {
        if (button_press_start == 0) {
            button_press_start = millis(); // Start timing the button press
        } else if (millis() - button_press_start >= 3000) {
            // If button is held for 3 seconds
            led_mode = !led_mode; // Toggle LED mode
            if (led_mode) {
                flash_led(GREEN_PIN); // Flash green to indicate mode on
            } else {
                flash_led(RED_PIN); // Flash red to indicate mode off
            }
            button_press_start = 0; // Reset button press timer
            delay(500); // Debounce delay
        }
    } else {
        button_press_start = 0; // Reset if button is released
    }

    // Flashing LEDs for new signals found
    if (ble_found > 0 && led_mode) {
        for (int i = 0; i < ble_found; i++) {
            flash_led(BLUE_PIN); // Flash blue for each Bluetooth signal found
        }
    }
}
        ...
        if (s2buf.length() >= 2) {
            ...
            if (last_sim_request == 0 || millis() - last_sim_request > 15000 || had_gsm_data == true) {
                if (!using_bw16) {
                    ...
                    Serial.println("Requesting data from SIM");
                    if (led_mode) {
                        flash_led(RED_PIN); // Flash red for cellular
                    }
                } else {
                    Serial2.print("ATWS\r\n");
